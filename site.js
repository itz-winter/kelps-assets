...
(function () {
  ...
  function injectAssetStyles() {
    ...
    s.textContent = [
      ...
      ".asset-card-link{display:inline-flex;align-items:center;gap:.3rem;padding:.4rem .9rem;border:1px solid var(--border);border-radius:var(--radius-sm);font-size:.8rem;color:var(--brand-primary);text-decoration:none;white-space:nowrap;transition:border-color .15s,background .15s}",
      ".asset-card-link:hover{border-color:var(--brand-primary);background:var(--brand-light);text-decoration:none}",
      ".asset-list{display:flex;flex-direction:column;gap:1rem;margin:1.25rem 0}",
      ".asset-card-cdn{font-size:.78rem;color:var(--text-muted);margin:.25rem 0 0;display:block;word-break:break-all}",
  ...
  function makeAssetCard(item) {
    ...
    if (item.href) {
      ...
      c.title = "Click to copy URL";
      c.className = "asset-card-cdn";
      c.textContent = "https://assets.kelpw.ing" + item.href;
    ...
    fetch("search-index.json")
      ...
      .then(function (data) {
        var items = data.filter(function (i) {
          return i.section === slug;
        });
        if (items.length === 0) {
          var note = document.createElement("div");
          note.className = "callout callout-note";
          note.innerHTML =
            "<strong>Coming Soon</strong><p>No assets have been uploaded yet. Check back later!</p>";
        ...
        order.forEach(function (name) {
          ...
          var useCards = groupItems.some(function (i) {
            return i.display === "card";
          });
          ...
          if (useCards) {
            ...
            groupItems.forEach(function (item) {
              list.appendChild(makeAssetRow(item));
            });
      ...
      .catch(function () {
        ...
        err.innerHTML =
          "<strong>Error</strong><p>Could not load asset index.</p>";
  ...
  var THEMES = {
    ...
    ocean: {
      name: "Ocean",
      swatch: ["#7eb8ff", "#1e2d3d"],
      dark: true,
      vars: {
        "--brand-primary": "#7eb8ff",
        "--brand-dark": "#5a9de8",
  ...
  function buildCustomVars(accentHex, bgHex) {
    ...
    var mix = dark
      ? function (t) {
          return _toHex(_blend(bg, w, t));
        }
      : function (t) {
          return _toHex(_blend(bg, k, t));
        };
    return {
      ...
      "--bg-inline": mix(0.13),
      "--bg-tip": dark
        ? _toHex(_blend(bg, [0, 60, 0], 0.35))
        : _toHex(_blend(bg, [0, 60, 0], 0.08)),
      "--bg-note": dark
        ? _toHex(_blend(bg, [0, 0, 80], 0.35))
        : _toHex(_blend(bg, [0, 0, 80], 0.08)),
      "--bg-warn": dark
        ? _toHex(_blend(bg, [80, 60, 0], 0.35))
        : _toHex(_blend(bg, [80, 60, 0], 0.08)),
      "--bg-danger": dark
        ? _toHex(_blend(bg, [80, 0, 0], 0.35))
        : _toHex(_blend(bg, [80, 0, 0], 0.08)),
      "--text-primary": dark ? "#f0f4f8" : "#1a1a1a",
      "--text-secondary": dark
        ? _toHex(_blend(a, w, 0.5))
        : _toHex(_blend(a, k, 0.35)),
      "--text-muted": dark
        ? _toHex(_blend(a, [100, 100, 100], 0.5))
        : "#6b6b6b",
  ...
  function saveTheme(key, data) {
    try {
      localStorage.setItem(
        "adoc-theme",
        JSON.stringify({ key: key, data: data }),
      );
    } catch (e) {}
  }
    ...
    Object.keys(THEMES).forEach(function (key) {
      ...
      sw.addEventListener("click", function () {
        ...
        Object.keys(swatchEls).forEach(function (k) {
          swatchEls[k].classList.remove("active");
        });
